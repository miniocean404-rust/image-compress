...
pub unsafe fn get_explore_root_window() -> Option<HWND> {
    ...
    let cabinet_hwnd = FindWindowW(PCWSTR(class_name.as_ptr()), None);

    if cabinet_hwnd != HWND(0) {
    ...
    let class_name = "ExploreWClass"
        ...
        .chain(std::iter::once(0))
        .collect::<Vec<u16>>();
    let explore_hwnd = FindWindowW(PCWSTR(class_name.as_ptr()), None);

    if explore_hwnd != HWND(0) {
